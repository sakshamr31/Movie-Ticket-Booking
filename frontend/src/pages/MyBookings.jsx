        ...
        <div key={index} className='flex flex-col md:flex-row justify-between bg-primary/8 border border-primary/20 rounded-lg mt-4 p-2 max-w-3xl'>
          ...
          <div className='flex flex-col md:items-end md:text-right justify-between p-4'>
            ...
            <div className='flex items-center gap-4'>
              <p className='text-2xl text-emerald-700 font-semibold mb-3 bg-blue-300 px-2 rounded-md'>{currency}{item.amount}</p>

              {
                !item.isPaid && 
                <Link to={item.paymentLink} className='bg-red-500 px-4 py-1.5 mb-3 text-sm hover:bg-amber-700 rounded-full font-medium cursor-pointer'>Pay Now</Link>
              }
            </div>
